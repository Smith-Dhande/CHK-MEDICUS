...
export default function DnaCanvas({ className = '' }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numParticles; i++) {
      const pSpeed = Math.random() * 0.005 + 0.002;
      particles.push({
        ...
        speed: pSpeed,
        cosSpeed: Math.cos(pSpeed),
        sinSpeed: Math.sin(pSpeed)
      });
    }
    ...
    const render = () => {
      ...
      particles.forEach((p) => {
        ...
        const cosY = p.cosSpeed;
        const sinY = p.sinSpeed;
      ...
      // Combined Helix Loop (connecting rods and nodes in a single pass)
      for (let i = 0; i < numPoints; i++) {
        ...
        const x1 = cosTheta1 * radius;
        const z1 = sinTheta1 * radius;

        // Strand 2 (180 degree offset - mathematically negated)
        const x2 = -x1;
        const z2 = -z1;
